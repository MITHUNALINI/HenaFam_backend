...
public class HenaFeedbackService {
	...
	public ResponseEntity<HenaFeedback> createFeedback(HenaFeedback feedback) {
		...
		try {
			...
			Optional<Poultry> pou = poultryRepository.findById(feedback.getPoultryId());
			if(pou.isPresent()) {
				Poultry poultry = pou.get();
				int count = poultry.getRatecount() + 1;
				poultry.setRatecount(count); 
				poultry.setUserRate(poultry.getUserRate() + feedback.getRatting());
				poultry.setRatting(poultry.getUserRate()/poultry.getRatecount());
...
//	}
	...
	public ResponseEntity<List<HenaFeedback>> getFeedbackByProductId(String productId) {
		try {
			List<HenaFeedback> feedback = henaFeedbackRepository.findByPoultryId(productId);
			if(feedback.isEmpty()) {
				return new ResponseEntity<>(HttpStatus.NO_CONTENT);
			}else {
